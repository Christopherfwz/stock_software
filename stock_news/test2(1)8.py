...
def getnews(search_key, sizeup_ratio):
    ...
    for counter in range(len(abstract)):
        abstract[counter] = abstract[counter][2:][:-1]
    ...
    response = []
    response.extend(ratio)
    response.extend(abstract)
    # for counter in range(len(ratio)):
    #     response.extend(ratio[counter])
    #     response.extend(abstract[counter])
